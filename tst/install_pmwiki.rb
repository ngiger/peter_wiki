...
raise 'xx' unless File.exist?(CONFIG_PHP)
...
config_dir = File.join(INSTALL_DIR, 'local')
FileUtils.makedirs(config_dir, verbose: true)
FileUtils.cp(CONFIG_PHP, config_dir, verbose: true, preserve: true)
FileUtils.cp_r(ORIG_WIKI_D, INSTALL_DIR, verbose: true, preserve: true)
FileUtils.cp_r(ORIG_WIKI_D.sub('wiki.d', 'uploads'), INSTALL_DIR, verbose: true, preserve: true)
